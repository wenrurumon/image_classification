...
seeds2 <- function(seed){
  print(seed)
  set.seed(seed);trainsel <- subsample(length(d1),2)
  print(lapply(trainsel,function(x){tapply(d1[x],d1[x],length)}))
  print(lapply(trainsel,function(x){tapply(d2[x],d2[x],length)}))

  #Train and test for dataset1
  d <- d1; x <- x1; s <- trainsel; z <- 10
  di1 <- d[s[[1]]]
  xi1 <- x[s[[1]],]
  xi1.pca <- pca(xi1)
  mat.pca <- pca(xi1)$mat[,1:which(xi1.pca$prop>0.99)[1]]
  
  xi1 <- xi1%*%mat.pca
  z <- order(-abs(class_SDR(xi1,di1)[,1]))[1:z]
  
  mat.lda <- lda(di1~xi1[,z])$scaling
  pi1 <- (xi1[,z] %*% mat.lda)>0
  print(train1 <- table(di1,pi1))

  di2 <- d[s[[2]]]
  xi2 <- (x[s[[2]],]%*%mat.pca)[,z]
  pi2 <- (xi2 %*% mat.lda)>0
  print(test1 <- table(di2,pi2))
  
  #Train and test for dataset2
  d <- d2; x <- x2; s <- trainsel; z <- 10
  di1 <- d[s[[1]]]
  xi1 <- x[s[[1]],]
  xi1.pca <- pca(xi1)
  mat.pca <- pca(xi1)$mat[,1:which(xi1.pca$prop>0.99)[1]]
  ...
  mat.lda <- lda(di1~xi1[,z])$scaling
  pi1 <- (xi1[,z] %*% mat.lda)>0
  print(train2 <- table(di1,pi1))
  
  di2 <- d[s[[2]]]
  xi2 <- (x[s[[2]],]%*%mat.pca)[,z]
  pi2 <- (xi2 %*% mat.lda)>0
  print(test2 <- table(di2,pi2))
  
  #Output
  out.table <- list(train1,test1,train2,test2)
  out.eva <- c(seed,sapply(out.table,eva))
